...
| 에이전트 | 역할 | 입력 | 출력 |
|---|---|---|---|
| `extractor` | 사용자 질문에서 신체 상태 / 건강 고민 키워드 추출 | 사용자 질의 (`query`) | 쉼표 구분 키워드 (`conditions`) |
| `matcher` | 추출된 상태를 해결할 수 있는 운동 5가지 추천 | 신체 상태 키워드 (`conditions`) | 쉼표 구분 운동 목록 (`exercise_candidates`) |
| `answer` | 상태 + 운동 리스트를 개조식 포맷으로 최종 답변 생성 | `conditions` + `exercise_candidates` | 개조식 최종 답변 (`result`) |
